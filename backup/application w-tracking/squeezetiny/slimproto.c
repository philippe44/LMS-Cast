	...
	case 'p':
		{
			...
			LOG_INFO("[%p] pause interval (0 ignored): %u", ctx, interval);
		}
		break;
	case 'a':
		{
	...
	case 'u':
		{
			...
			if (ctx->output.state == OUTPUT_STOPPED) {
				if (ctx->output.render == RD_STOPPED) ctx->output.state = OUTPUT_TRACKING;
				else ctx->output.state = OUTPUT_WAITING;
			}
	...
	case 's':
		{
			...
			output_start(ctx);
			codec_open(uri.codec, uri.sample_size, uri.sample_rate, uri.channels, uri.endianness, ctx);
			stream_sock(ip, port, header, header_len, strm->threshold * 1024, ctx->autostart >= 2, ctx);

			LOCK_O;

			uri.port = ctx->output.port;
			sprintf(uri.name, "bridge-%hu", ctx->output.index++);

			rc = ctx_callback(ctx, SQ_SET_TRACK, NULL, &uri);
	...
	while (ctx->running && !ctx->new_server) {
		...
		if (wake || now - ctx->slim_run.last > 100 || ctx->slim_run.last > now) {
			...
			*/
			...
			if (ctx->decode.state == DECODE_COMPLETE || ctx->decode.state == DECODE_ERROR && ctx->status.output_full == 0) {
