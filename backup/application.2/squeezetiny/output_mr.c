...
static void output_thread(struct thread_ctx_s *ctx) {
	...
	while (ctx->output_running) {
		...
		if (_buf_used(ctx->outputbuf)) {
			...
			if (sent > 0) {
				...
				if (!bytes) {
					if (ctx->output.track_start != ctx->outputbuf->readp) {
						LOG_ERROR("[%p] not starting on a track boundary", ctx);
					}
			...
			// reaching the next track boundary, end of streaming
			if (ctx->output.track_start == ctx->outputbuf->readp) {
				LOG_INFO("[%p]: completed, reaching boundary: %u bytes ", ctx, bytes);
				done = true;
			}
		} else {
			// check if all sent and no next track
			LOCK_D;
		...
		// track sent, exit thread (will be joined)
		if (done) break;
	}
