...
/*----------------------------------------------------------------------------*/
char*	GetLocalIP(void) { return glIPaddress; };

/*----------------------------------------------------------------------------*/
u16_t	GetLocalPort(void) { return glPort; };
...
bool sq_callback(sq_dev_handle_t handle, void *caller, sq_action_t action, u8_t *cookie, void *param)
{
	...
	switch (action) {
		...
		case SQ_SET_TRACK: {
			struct track_param *p = (struct track_param_t*) param;
			...
			asprintf(&uri, "http://%s.%s", p->uri, codec2ext(p->codec));
			if (p->codec == 'p') strcpy(p->mime, "audio/wav");
			p->duration = Device->MetaData.duration;

			if (Next) {
				// to know what is expected next
				strcpy(Device->NextMime, p->mime);
				Device->NextURI = uri;
				LOG_INFO("[%p]: next URI %s", Device, Device->NextURI);
			}
			else {
				rc = CastLoad(Device->CastCtx, uri, p->mime, (
							  Device->Config.SendMetaData) ? &Device->MetaData : NULL);
				sq_free_metadata(&Device->MetaData);
				LOG_INFO("[%p]: current URI %s", Device, uri);
			}
