...
static void output_thru_thread(struct thread_ctx_s *ctx) {
	...
	while (ctx->mr_running) {
		...
		if (_buf_used(ctx->streambuf)) {
			...
			if (out->live && ctx->config.stream_pacing_size != -1 &&
		...
		if (out->write_file && ctx->stream.state <= DISCONNECT &&
			...
			LOG_INFO("[%p]:[%u] wrote total %Ld", ctx, out->idx, out->write_count_t);
...
/*---------------------------------------------------------------------------*/
void output_mr_thread_init(unsigned outputbuf_size, struct thread_ctx_s *ctx) {
	pthread_attr_t attr;
