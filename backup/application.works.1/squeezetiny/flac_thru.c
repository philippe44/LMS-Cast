...
decode_state flac_decode(struct thread_ctx_s *ctx) {
	...
	if (p->streaminfo) {
		...
		if (strncmp((u8_t*) &frame, "fLaC", 4) && create_streaminfo(&frame, p->streaminfo, &p->sample_rate)) {
...
/*---------------------------------------------------------------------------*/
static void flac_open(struct track_param *param, struct thread_ctx_s *ctx) {
	struct flac *p = ctx->decode.handle;
...
/*---------------------------------------------------------------------------*/
static char* flac_mime(struct thread_ctx_s *ctx) {
	struct flac *p = ctx->decode.handle;
...
/*---------------------------------------------------------------------------*/
static u64_t flac_size(struct thread_ctx_s *ctx) {
	return 0;
}
...
struct codec *register_flac(void) {
	static struct codec ret = {
		...
		flac_close,  // close
		flac_decode, // decode
		flac_mime,	 // get mime type
		flac_size,	 // get total length, 0 is unknown
	};
