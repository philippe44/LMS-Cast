...
pointers coudl be reset at the begining every time an output exit because no
	...
	case 's':
		{
			...
			output_start(ctx);
			sprintf(info.uri, "%s:%hu/bridge-%hu", GetLocalIP(), ctx->output.port, ++ctx->output.index);

			if (strm->format != 'a')
				info.sample_size = (strm->pcm_sample_size != '?') ? pcm_sample_size[strm->pcm_sample_size - '0'] : 0xff;
			else
				info.sample_size = strm->pcm_sample_size;

			info.sample_rate = (strm->pcm_sample_rate != '?') ? pcm_sample_rate[strm->pcm_sample_rate - '0'] : 0xff;
			if (info.sample_rate > ctx->config.sample_rate) {
				 LOG_WARN("[%p]: Sample rate %u error suspected, forcing to %u", ctx, info.sample_rate, ctx->config.sample_rate);
				 info.sample_rate = ctx->config.sample_rate;
			}

			info.channels = (strm->pcm_channels != '?') ? pcm_channels[strm->pcm_channels - '1'] : 0xff;
			info.endianness = (strm->pcm_endianness != '?') ? strm->pcm_endianness - '0' : 0xff;
			info.codec = strm->format;
			if (info.codec == 'p')
				sprintf(info.mime, "audio/L%u;rate=%u;channels=%u", info.sample_size,
									info.sample_rate, info.channels);
			else
				strcpy(info.mime, codec2mime(info.codec));

			if (strm->format != '?') {
				if (!ctx_callback(ctx, SQ_SET_TRACK, NULL, &info)) sendSTAT("STMn", 0, ctx);
				else codec_open(&info, ctx);
			} else if (ctx->autostart >= 2) {
				// extension to slimproto to allow server to detect codec from
				// response header and send back in codc message
...
static void process_codc(u8_t *pkt, int len, struct thread_ctx_s *ctx) {
	...
	if (codc->format != 'a')
		info.sample_size = (codc->pcm_sample_size != '?') ? pcm_sample_size[codc->pcm_sample_size - '0'] : 0xff;
	else
		info.sample_size = codc->pcm_sample_size;
	info.sample_rate = (codc->pcm_sample_rate != '?') ? pcm_sample_rate[codc->pcm_sample_rate - '0'] : 0xff;
	info.channels = (codc->pcm_channels != '?') ? pcm_channels[codc->pcm_channels - '1'] : 0xff;
	info.endianness = (codc->pcm_endianness != '?') ? codc->pcm_channels - '0' : 0xff;
	info.codec = codc->format;
	if (info.codec == 'p')
		sprintf(info.mime, "audio/L%u;rate=%u;channels=%u", info.sample_size,
						 info.sample_rate, info.channels);
	else
...
static void slimproto_run(struct thread_ctx_s *ctx) {
	...
	while (ctx->running && !ctx->new_server) {
		...
		if (wake || now - ctx->slim_run.last > 100 || ctx->slim_run.last > now) {
			...
			ctx->status.ms_played = ctx->output.ms_played;
			...
			if (ctx->output.track_started && ctx->output.render == RD_PLAYING) {
				_sendSTMs = true;
				ctx->sentSTMs = true;
				/*
				state has been consumed, so now needs a state change to be usable
				again. Especially important for the auto track change which must
				capture another RD_PLAYING event
				*/
				ctx->output.render = RD_ACQUIRE;
				ctx->output.track_started = false;
			}

			if (ctx->output.state == OUTPUT_RUNNING && !ctx->sentSTMu &&
				ctx->status.output_full == 0 && ctx->status.stream_state <= DISCONNECT &&
			...
			/*
			// STMo cannot not really be detected, need to think about that
			if (ctx->output.state == OUTPUT_RUNNING && !ctx->sentSTMo &&
				ctx->status.output_full == 0 && ctx->status.stream_state == STREAMING_HTTP &&
			...
			*/
			...
			if ((ctx->status.stream_state == STREAMING_HTTP || ctx->status.stream_state == STREAMING_FILE ||
				...
				if (ctx->output.render == RD_STOPPED) ctx_callback(ctx, SQ_PLAY, NULL, NULL);
				// autostart 2 and 3 require cont to be received first
			}
			if ((ctx->decode.state == DECODE_COMPLETE && ctx->status.output_full == 0 && ctx->sentSTMs) ||
