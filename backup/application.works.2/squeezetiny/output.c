...
int _output_cont_bytes(struct thread_ctx_s *ctx) {
	return ctx->output.thru ? _buf_cont_read(ctx->outputbuf) :_buf_cont_read(ctx->encodebuf);
}
...
ssize_t output_pcm_header(void **header, size_t *hsize, struct thread_ctx_s *ctx ) {
	...
	sample_size = (out->sample_size == 24 && ctx->config.L24_format == L24_TRUNC_16) ? 16 : out->sample_size;
	...
	*/
	if (out->duration) {
		...
		length = duration.quot * (u64_t) out->sample_rate * (u64_t) (sample_size/8) * (u64_t) out->channels;
		length += (duration.rem * (u64_t) out->sample_rate * (u64_t) (sample_size/8) * (u64_t) out->channels) / 1000;
		size = length;
	} else {
		...
		} else length = size = ctx->config.stream_length;
	}
	...
	case 'w': {
		...
		little32(&h->subchunk2_size, size);
		little32(&h->chunk_size, 36 + size);
   ...
   case 'i': {
		...
		big32(&h->data_size, size + 8);
		big32(&h->chunk_size, (size+8+8) + (18+8) + 4);
		big32(&h->frames, size / out->sample_rate);
		if (length) length += (8+8) + (18+8) + 4 + 8;
		*header = h;
		*hsize = sizeof(struct aiff_header_s) - AIFF_PAD_SIZE;
		}
		break;
   case 'p':
		*header = NULL;
...
/*---------------------------------------------------------------------------*/
void truncate(u8_t *dst, u8_t *src, size_t bytes, int in_endian, int out_endian)
{
