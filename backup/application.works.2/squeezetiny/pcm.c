...
static decode_state pcm_decode(struct thread_ctx_s *ctx) {
	...
	// truncate or swap
	if (ctx->output.sample_size == 24 && ctx->config.L24_format == L24_TRUNC_16) {
		truncate(optr, iptr, in, ctx->output.in_endian, ctx->output.out_endian);
		out = (out * 2) / 3;
	} else {
		if (ctx->output.in_endian != ctx->output.out_endian) swap(optr, iptr, in, ctx->output.sample_size);
		else memcpy(optr, iptr, in);
	}

	// apply gain if any
	apply_gain(optr, ctx->output.replay_gain, out,
			   (ctx->output.sample_size == 24 && ctx->config.L24_format == L24_TRUNC_16) ?
			   16 : ctx->output.sample_size,
			   ctx->output.out_endian);
