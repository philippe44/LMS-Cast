...
static void sendSTAT(const char *event, u32_t server_timestamp, struct thread_ctx_s *ctx) {
	...
	packN(&pkt.output_buffer_size, ctx->config.output_buf_size);
	packN(&pkt.output_buffer_fullness, ctx->config.output_buf_size / 2);
	...
	case 's':
		{
			...
			sq_get_metadata(ctx->self, &info.metadata, ctx->output.state == OUTPUT_RUNNING);
			...
			if (strm->format != '?') {
				...
				strcpy(out->mime, select_mime(out->codec, ctx->mimecaps, ctx->config.encode));
				out->format = mime2format(out->mime);
				out->out_endian = (out->format == 'w');
				out->length = ctx->config.stream_length;
				if (!strcasecmp(ctx->config.encode, "thru")) out->thru = true;
				else out->thru = false;

				codec_open(out->codec, out->sample_size, out->sample_rate, out->channels, out->in_endian, ctx);

				strcpy(info.mime, out->mime);
				sprintf(info.uri, "http://%s:%hu/bridge-%hu.%s", GetLocalIP(),
						ctx->output.port, ++ctx->output.index, codec2ext(out->codec));
...
static void process_codc(u8_t *pkt, int len, struct thread_ctx_s *ctx) {
	...
	if (!ctx_callback(ctx, SQ_SET_TRACK, NULL, &info)) sendSTAT("STMn", 0, ctx);
	else codec_open(out->codec, out->sample_size, out->sample_rate, out->channels, out->in_endian, ctx);
...
static void slimproto_run(struct thread_ctx_s *ctx) {
	...
	while (ctx->running && !ctx->new_server) {
		...
		if (wake || now - ctx->slim_run.last > 100 || ctx->slim_run.last > now) {
			...
			// not a perfect overrun detection
			if (ctx->output.state == OUTPUT_RUNNING && !ctx->sentSTMo && ctx->sentSTMs &&
