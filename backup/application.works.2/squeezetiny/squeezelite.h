...
ssize_t		output_pcm_header(void **header, size_t *hsize, struct thread_ctx_s *ctx );
void 		swap(u8_t *src, u8_t *dst, size_t bytes, u8_t size);
void 		truncate(u8_t *src, u8_t *dst, size_t bytes, int in_endian, int out_endian);
void 		apply_gain(void *p, u32_t gain, size_t bytes, u8_t size, int endian);

int 		_output_bytes(struct thread_ctx_s *ctx);
int 		_output_cont_bytes(struct thread_ctx_s *ctx);
...
struct outputstate {
	...
	char	format;			// format to be sent to player (p=pcm, w=wav, i=aif, f=flac)
	bool	thru;			// thru mode, just pass data to player
	u16_t   index;			// file number sent in URI, increments by 1 for every track
	int		http;			// listening socket number for http server
	u16_t	port;           // listening port number for http server
	u8_t sample_size, channels, codec;	// per name, in original stream
	u32_t sample_rate;		// idem
	int in_endian, out_endian;	// 1 = little (MSFT/INTL), 0 = PCM/AAPL
	u32_t duration;         // track duration in ms, O if ignored
	bool  remote;			// local track or not (if duration == 0 => live)
	ssize_t length;			// HTTP content-length (-1:no chunked, -3 chunked if possible)
	bool chunked;			// chunked mode
	char mime[_STR_LEN_];	// content-type to send to player
	u32_t	ms_played;		// elapsed time in ms
	bool  track_started;	// track has started to be streamed
	u8_t  *track_start;     // pointer where track starts in buffer, almost unused
	enum { RD_ACQUIRE = -1, RD_STOPPED, RD_PLAYING, RD_PAUSED }	render; // player known state
