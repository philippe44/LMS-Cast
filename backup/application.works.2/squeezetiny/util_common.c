...
/*---------------------------------------------------------------------------*/
char *codec2mime(char codec)
{
	switch (codec) {
		case 'm': return  "audio/mpeg";
		case 'f': return  "audio/flac";
		case 'w': return  "audio/wma";
		case 'o': return  "audio/ogg";
		case 'a': return  "audio/aac";
		case 'l': return  "audio/m4a";
		case 'p': return  "audio/wav";
		default: return "";
	}
}

/*---------------------------------------------------------------------------*/
char *codec2ext(u8_t format)
{
	switch(format) {
		case 'p': return "wav";
		case 'm': return "mp3";
...
/*---------------------------------------------------------------------------*/
u8_t mime2format(char *mime)
{
	if (!mime) return '\0';

	if (strstr(mime, "wav")) return 'w';
	if (strstr(mime, "flac") || strstr(mime, "flc")) return 'f';
	if (strstr(mime, "mp3") || strstr(mime, "mpeg")) return 'm';
	if (strstr(mime, "ogg")) return 'o';
	if (strstr(mime, "aif")) return 'i';

	return '\0';
}

/*---------------------------------------------------------------------------*/
char *select_mime(char in, char *caps[], char *out) {
	char *mime = codec2mime(in);

	while (caps) {
		if (!strcmp(mime, *caps)) return *caps;
		caps++;
   }
