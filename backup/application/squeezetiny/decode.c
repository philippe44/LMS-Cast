...
struct codec	*codecs_thru[MAX_CODECS];
...
/*---------------------------------------------------------------------------*/
void codec_open(bool thru, struct track_param *param, struct thread_ctx_s *ctx) {
	int i;

	LOG_DEBUG("codec open: '%c'", param->codec);
	...
	for (i = 0; i < MAX_CODECS; ++i) {
		...
		if (codecs[i] && (codecs[i]->id == param->codec || codecs[i]->id == '*')) {
